...
server/
  ...
  db.js             — pg Pool configured from DATABASE_URL; supports AWS Aurora/RDS SSL (RDS_SSL_CA env var) and configurable pool settings (DB_POOL_MAX, DB_IDLE_TIMEOUT, DB_CONNECTION_TIMEOUT, DB_STATEMENT_TIMEOUT)
  ...
  export-data.js    — Database export script (SQL format by default for Aurora/RDS import, --json for legacy JSON format)
